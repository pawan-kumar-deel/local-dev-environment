...
export const useEnvironmentVariables = () => {
    const [envVarsAnchorEl, setEnvVarsAnchorEl] = useState(null);
    ...
    useEffect(() => {
        ...
        const filtered = allEnvVars.filter(env => env.name.toLowerCase().includes(term) || env.value.toLowerCase().includes(term));
        setFilteredEnvVars(filtered);
    }, [searchTerm, allEnvVars]);
    const handleEnvVarsClick = (event, pod) => {
